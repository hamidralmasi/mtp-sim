...
DWRRQueueDisc::DoEnqueue (Ptr<QueueDiscItem> item)
{
    ...
    if (itr == m_DWRRs.end ())
    {
        ...
        DropBeforeEnqueue (item, "Cannot find class, dropping the packet");//Drop (item);
        return false;
    }
    ...
    {
        DropBeforeEnqueue (item, "Cannot enqueue, dropping the packet");//Drop (item);
        return false;
    }
...
DWRRQueueDisc::DoDequeue (void)
{
    ...
    while (true)
    {
        ...
        //Ptr<const Ipv4QueueDiscItem> ipv4Item = Ptr<const Ipv4QueueDiscItem> (dynamic_cast<const Ipv4QueueDiscItem *> (PeekPointer (item)));
        ...
        uint32_t length = item->GetSize ();//uint32_t length = ipv4Item->GetPacketSize ();
