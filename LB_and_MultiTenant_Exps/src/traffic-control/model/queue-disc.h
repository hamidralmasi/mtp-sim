...
public:
  ...
  //Showje-NFlows
  uint32_t GetNFlows (void) const;
  void UpdateNFlows(void); 
  ...
  bool EnqueueWithToS (Ptr<QueueDiscItem> item, int ToS);
  ...
  Ptr<QueueDiscItem> DequeueWithToS (int ToS);
...
private:
  ...
  //Showje-NFlows
  TracedValue<uint32_t> m_nFlows;   //!< Number of Flows in the queue
  std::map<uint32_t, uint32_t> m_flowPacketsMap;   //!< Number of packets each flow(hash of 5-tuple) has in the queue
