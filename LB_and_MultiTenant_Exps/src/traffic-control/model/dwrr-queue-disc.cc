...
DWRRQueueDisc::DoDequeue (void)
{
    ...
    while (true)
    {
        ...
        if (length <= dwrrClass->deficit && cl == 0)
        {
            dwrrClass->deficit -= length;
            NS_LOG_LOGIC ("DEFICIT:      CLASS 00000000000    " << dwrrClass->deficit);
        ...
        if (length <= dwrrClass->deficit && cl == 1)
        {
            dwrrClass->deficit -= length;
            NS_LOG_LOGIC ("DEFICIT:      CLASS 111111111111    " << dwrrClass->deficit);
            Ptr<QueueDiscItem> retItem = dwrrClass->qdisc->  Dequeue();
            if (retItem == 0)
            {
                return 0;
        ...
        NS_LOG_LOGIC ("Pushing back class with quantum: " << dwrrClass->quantum << " and deficit: " << dwrrClass->deficit);
    }
