...
TypeId QueueDisc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::QueueDisc")
    ...
    .AddTraceSource ("BytesInQueue",
                     ...
                     "ns3::TracedValueCallback::Uint32")
    //Showje_Nflows
    .AddTraceSource ("FlowsInQueue",
                     "Number of flows currently using the queue disc",
                     MakeTraceSourceAccessor (&QueueDisc::m_nFlows),
                     "ns3::TracedValueCallback::Uint32")
...
void 
QueueDisc::UpdateNFlows()
{
  NS_LOG_FUNCTION (this);
  std::map<uint32_t, uint32_t>::iterator it = m_flowPacketsMap.begin();
  int liveFlowCount = 0;
  int inBurst = 0;
  int largeQueueThresh = 116;
  while (it != m_flowPacketsMap.end()){
        //NS_LOG_INFO("Number of packets for flow " << it->first << " is " << it->second);
        if (it->second > 0){
            liveFlowCount++;
            if (m_nPackets > largeQueueThresh){
                inBurst++;
                if (inBurst == 1){
                    NS_LOG_INFO("");
                    NS_LOG_INFO("##############################   UpdatingNFlows   ##############################");
                    NS_LOG_INFO("");
                }
                NS_LOG_INFO("Total packets " << m_nPackets  << " Number of packets for flow with hash "<< it->first << " is " << it->second);
            }
            ++it;
        }
        else {
            m_flowPacketsMap.erase(it++);
        }
 }
  m_nFlows = liveFlowCount;
  if (m_nPackets > largeQueueThresh){
        NS_LOG_INFO("Number of live flows is: " << m_nFlows);
  }
}
uint32_t
...
QueueDisc::Drop (Ptr<QueueDiscItem> item)
{
  ...
  m_nPackets--;
  m_flowPacketsMap[item->Hash()] = m_flowPacketsMap[item->Hash()] - 1;
  UpdateNFlows();
...
QueueDisc::Enqueue (Ptr<QueueDiscItem> item)
{
  ...
  m_traceEnqueue (item);
  //NS_LOG_INFO ("Showje-NFlows number of packets for flow " << item->Hash() << " is " << m_flowPacketsMap[item->Hash()]);
  return DoEnqueue (item);
}

bool
QueueDisc::EnqueueWithToS (Ptr<QueueDiscItem> item, int ToS)
{
  NS_LOG_FUNCTION (this << item);
  ...
  //NS_LOG_INFO ("Showje-NFlows number of packets for flow " << item->Hash() << " is " << m_flowPacketsMap[item->Hash()]);
  return DoEnqueue (item);
  //return DoEnqueueWithToS (item, ToS);
}
...
Ptr<QueueDiscItem>
QueueDisc::DequeueWithToS (int ToS)
{
  NS_LOG_FUNCTION (this);

  Ptr<QueueDiscItem> item;
  item = DoDequeue ();
  //item = DoDequeueWithToS (ToS);
