...
public:
  ...
  bool EnqueueWithToS (Ptr<QueueItem> item, int ToS);
  Ptr<QueueItem> DequeueWithToS (int ToS);
...
private:
  ...
  virtual bool DoEnqueueWithToS (Ptr<QueueItem> item, int ToS) = 0;
  virtual Ptr<QueueItem> DoDequeueWithToS (int ToS) = 0;
  /**
