  ...
  if (item != 0)
    {
      ...
      int tmp = item->GetPacketSize();
      if (item->GetPacketSize() > tmp){
        m_nBytes -= tmp;
        m_nPackets--;
      }
      else 
...
bool 
Queue::EnqueueWithToS (Ptr<QueueItem> item, int ToS)
{
  NS_LOG_FUNCTION (this << item);
  Ptr<Packet> p = item->GetPacket ();
  ...
  // If DoEnqueue fails, Queue::Drop is called by the subclass
  //
  bool retval = DoEnqueueWithToS (item, ToS);
  if (retval)
    {
      NS_LOG_LOGIC ("m_traceEnqueue (p)");
      m_traceEnqueue (item->GetPacket ());
...
Queue::DequeueWithToS (int ToS)
{
  ...
  Ptr<QueueItem> item = DoDequeueWithToS (ToS);
