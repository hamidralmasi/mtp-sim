  ...
  static TypeId tid = TypeId ("ns3::Ipv4GlobalRouting")
    ...
    .AddAttribute("PerflowEcmpRouting",
                  "Set to true if flow are randomly routed among ECMP",
                   ...
                   MakeBooleanChecker ())
    .AddAttribute ("AlternateLinkRouting",
                   "Set to true when packets should use the alternate path; set to false for default first interface reaching the destination",
                   BooleanValue (false),
                   MakeBooleanAccessor (&Ipv4GlobalRouting::m_alternateLinkRouting),
                   MakeBooleanChecker ())
    .AddAttribute ("SourceRouting",
                   "Set to true when packets should be source routed; set to false for default first interface reaching the destination",
                   BooleanValue (false),
                   MakeBooleanAccessor (&Ipv4GlobalRouting::m_sourceRouting),
                   MakeBooleanChecker ())
    .AddAttribute ("LoadAwareRouting",
                   "Set to true when packets should be sent to the least loaded (queue) interface; set to false for default first interface reaching the destination",
                   BooleanValue (false),
                   MakeBooleanAccessor (&Ipv4GlobalRouting::m_loadAwareRouting),
                   MakeBooleanChecker ())
    .AddAttribute ("ReqLoadAwareRouting",
                   "Set to true when packets should be sent to the least loaded (queue) interface; set to false for default first interface reaching the destination",
                   BooleanValue (false),
                   MakeBooleanAccessor (&Ipv4GlobalRouting::m_reqLoadAwareRouting),
                   MakeBooleanChecker ())            
...
Ipv4GlobalRouting::Ipv4GlobalRouting ()
  : m_randomEcmpRouting (false),
    ...
    m_alternateLinkRouting (false),
    m_sourceRouting (false),
    m_loadAwareRouting (false),
    m_reqLoadAwareRouting (false),
...
Ipv4GlobalRouting::LookupGlobal (Ipv4Address dest, Ptr<Packet> packet, const Ipv4Header &header, uint32_t flowId, Ptr<NetDevice> oif)
{
  ...
  Ptr<Ipv4Route> rtentry0 = 0;
  Ptr<Ipv4Route> rtentry1 = 0;
  ...
  if (allRoutes.size () > 0 ) // if route(s) is found
    {
      ...
      if (m_randomEcmpRouting)
      ...
      else if (m_perFlowEcmpRouting && flowId != 0 && !m_reqLoadAwareRouting) // If the flow id is 0, it may be the socket setup endpoint request, we simply return the first
        {                                           // available route to indicate the address is not local
          ...
          if (srcPort == 22222 || destPort == 22222 || srcPort == 44444 || destPort == 44444)
          {
            selectIndex = 0;
          }
          else if (destPort % 2 == 1 && allRoutes.size() == 2)
          {
            selectIndex = 1;
          }
          NS_LOG_LOGIC ("Per flow ECMP is enabled, select index: " << selectIndex << " for flow: " << flowId);
        }
      else if (m_loadAwareRouting && packet != 0)
        {
          selectIndex = 0;
          uint32_t minQueueSize = UINT32_MAX;
          uint32_t queueSize;
          for (uint32_t interface = 0; interface < allRoutes.size(); interface++)
          {
            ...
            const Ptr<NetDevice> netDevice = m_ipv4->GetNetDevice (allRoutes[interface]->GetInterface ());
            ...
            if (queueDisc != 0)
            {
              queueSize = queueDisc->GetNBytes ();
              if (queueSize < minQueueSize)
              {
                minQueueSize = queueSize;
                selectIndex = interface;
              }
            }
          }
        }
      else if (m_reqLoadAwareRouting)
        {
          selectIndex = 0;
          ...
          if(allRoutes.size() == 2)
          {
            ...
            uint16_t srcPort = tcpHeader.GetSourcePort();
            uint16_t destPort = tcpHeader.GetDestinationPort();
            if (srcPort == 22222 || destPort == 22222)
            {
              selectIndex = 0;
            }
            else
