...
Gnuplot2dDataset queuediscFlowCount;
...
DoGnuPlot (std::string id, AQM aqm)
{
    ...
    Gnuplot queuediscFlowCountGnuplot (GetFormatedStr (id, "FlowCount", "png", aqm).c_str ());
    queuediscFlowCountGnuplot.SetTitle ("FlowCount");
    queuediscFlowCountGnuplot.SetTerminal ("png");
    queuediscFlowCountGnuplot.AppendExtra ("set yrange [0:+120]");
    queuediscFlowCountGnuplot.AddDataset (queuediscFlowCount);
    std::ofstream queuediscFlowCountGnuplotFile (GetFormatedStr (id, "FlowCount", "plt", aqm).c_str ());
    queuediscFlowCountGnuplot.GenerateOutput (queuediscFlowCountGnuplotFile);
    queuediscFlowCountGnuplotFile.close ();
...
CheckQueueDiscSize (Ptr<QueueDisc> queue)
{
    ...
    uint32_t numFlows = queue->GetNFlows ();
    double timeNow = Simulator::Now ().GetSeconds ();
    queuediscDataset.Add (timeNow, qSize);
    queuediscFlowCount.Add (timeNow, numFlows);
...
int main (int argc, char *argv[])
{
    ...
    queuediscFlowCount.SetTitle ("FlowCount");
    queuediscFlowCount.SetStyle (Gnuplot2dDataset::LINES_POINTS);
