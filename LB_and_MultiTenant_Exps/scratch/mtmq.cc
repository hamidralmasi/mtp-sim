...
std::ofstream queueLengthT1;
std::ofstream queueLengthT2;
int numFlowsT1=10;
int numFlowsT2=80;
static void
...
void
CheckQueueSizeT1 (Ptr<QueueDisc> queue)
{
  uint32_t qSize = queue->GetNPackets ();
  uint32_t qSizeBytes = queue->GetNBytes ();
  queueLengthT1 << std::fixed << Simulator::Now ().GetSeconds () << " " << qSize << " " << qSizeBytes << std::endl;
  // check queue size every 1/1000 of a second
  Simulator::Schedule (MilliSeconds (1), &CheckQueueSizeT1, queue);
}

void
CheckQueueSizeT2 (Ptr<QueueDisc> queue)
{
  uint32_t qSize = queue->GetNPackets ();
  uint32_t qSizeBytes = queue->GetNBytes ();
  queueLengthT2 << std::fixed << Simulator::Now ().GetSeconds () << " " << qSize << " " << qSizeBytes << std::endl;
  // check queue size every 1/1000 of a second
  Simulator::Schedule (MilliSeconds (1), &CheckQueueSizeT2, queue);
}
...
GetFormatedStr (std::string id, std::string str, std::string terminal, AQM aqm, double endTime)
{
    ...
    if (aqm == RED)
    {
        //ss << "MTMQ_RED_" << id << "_" << str << "." << terminal;
        ss << "MTMQ_RED_" << id << "_" << str << "-" << endTime << "." << terminal;
...
int main (int argc, char *argv[])
{
    ...
    uint32_t q1Quantum = 1500;
    uint32_t q2Quantum = 1500;
    bool tracing = false;

#if 1
    LogComponentEnable ("MTMQ", LOG_LEVEL_INFO);
#endif
    ...
    cmd.AddValue("q1Quantum", "Red Min Threshold", q1Quantum);
    cmd.AddValue("q2Quantum", "Red Min Threshold", q2Quantum);
    ...
    NS_LOG_INFO ("q1Quantum:                     " << q1Quantum);
    NS_LOG_INFO ("q2Quantum:                     " << q2Quantum);
    ...
    dwrrQdisc->AddDWRRClass (queueDisc1, 0, q1Quantum);
    dwrrQdisc->AddDWRRClass (queueDisc2, 1, q2Quantum);
    ...
    for (uint32_t i = 0; i < 2; ++i)
    {
        if (i == 0) 
        {
            for (uint32_t j = 0; j < numFlowsT1; ++j)
            {
                ...
                Simulator::ScheduleNow (&CheckThroughput, pktSink, j);
            }
        }
        else
        {
    ...
    if (tracing)
    {
        ...
        traceFileName << "MTMQ-" << endTime << ".tr";
        p2p.EnableAsciiAll (ascii.CreateFileStream (traceFileName.str()));
        //p2p.EnablePcapAll ("MTMQ", false);
        //p2p.EnablePcap("MTMQ", switchToRecvNetDeviceContainer.Get (0));
    }

    std::stringstream cwndT1FileName;
    cwndT1FileName << "MTMQ-cwnd-T1-" << endTime <<".dat";
    ...
    cwndT2FileName << "MTMQ-cwnd-T2-" << endTime <<".dat";
    cwndStreamT2.open (cwndT2FileName.str(), std::ios::out);
    cwndStreamT2 << "#Time(s) Congestion Window (B)" << std::endl;

    std::stringstream queueLengthT1FileName;
    queueLengthT1FileName << "MTMQ-QueueLength-T1-" << endTime << ".dat";
    queueLengthT1.open (queueLengthT1FileName.str(), std::ios::out);
    queueLengthT1 << "#Time(s) QLen (Packets) QLen (Bytes)" << std::endl;

    std::stringstream queueLengthT2FileName;
    queueLengthT2FileName << "MTMQ-QueueLength-T2-" << endTime << ".dat";
